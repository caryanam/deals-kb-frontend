    ...
    try {
      if (user?.role === 'Dealer') {
        ...
        if (hasActivePlan) {
          ...
          // Save details first
          await submitRelistAfterPayment(listingId, formData);
          toast.info('Listing details saved! Redirecting to plan payment...');
        ...
        // Save details first
        await submitRelistAfterPayment(listingId, formData);
        toast.info('Listing details saved! Redirecting to relisting payment...');
        await triggerRelistPayment(listingId);
        setSuccessMsg('Please complete payment in the CCAvenue window to complete the relisting.');
        setTimeout(() => navigate(`${basePath}/my-listings`), 3000);
      }
    } catch (err) {
            ...
            <section className="card">
              ...
              {user?.role === 'Dealer' ? (() => {
                ...
                <div style={{
                  ...
                  <span style={{ color: '#7A2181', fontWeight: 800, fontSize: '0.9rem' }}>Listing Fee</span>
                  <strong style={{ color: '#1F1A1D', fontSize: '1.2rem' }}>₹{{ mobile: '11.80', laptop: '59.00', bike: '118.00', car: '590.00' }[productType?.toLowerCase()] || '590.00'}</strong>
                </div>
              )}
