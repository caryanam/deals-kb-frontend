  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>Payments</h1>
          <p style={{ color: '#64748b', margin: '0.35rem 0 0' }}>{roleLabel} payment history.</p>
        </div>
      ...
      {loading ? (
        <div style={{ padding: '3rem', display: 'flex', justifyContent: 'center' }}>
          <Loader2 size={30} style={{ animation: 'spin 1s linear infinite', color: '#2563eb' }} />
        </div>
      ) : (
          <section style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '0.75rem', overflow: 'hidden' }}>
            <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid #e2e8f0' }}>
              <h2 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, color: '#0f172a' }}>Payment History</h2>
            </div>
            {payments.length === 0 ? (
              <div style={{ padding: '2rem', color: '#64748b', textAlign: 'center' }}>No payments yet.</div>
            ) : (
                    ...
                    {payments.map((payment) => {
                      ...
                      return (
                        <tr key={payment.payment_id} style={{ borderTop: '1px solid #f1f5f9' }}>
                          <td style={{ padding: '0.85rem 1rem', fontWeight: 700, color: '#0f172a' }}>{payment.plan_name}</td>
                          <td style={{ padding: '0.85rem 1rem', color: '#475569' }}>{formatINR(payment.amount)}</td>
                          ...
                          <td style={{ padding: '0.85rem 1rem', color: '#64748b' }}>{payment.razorpay_order_id}</td>
                          <td style={{ padding: '0.85rem 1rem', color: '#64748b' }}>{payment.created_at ? new Date(payment.created_at).toLocaleString() : ''}</td>
                        </tr>
                      );
                    })}
                  </tbody>
